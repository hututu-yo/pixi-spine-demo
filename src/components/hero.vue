...
export default {
    ...
    methods: {
        init() {
            ...
            that.$_app.stop()
            that.$_app.loader
                // .add('goblins', `${this.publicPath}resourcev12/1015/101.json`)
                .add('goblins', `${this.publicPath}resourcev12/101_succbus.json`)
                // .add('goblins', `${this.publicPath}resourcev12/goblins.json`)
                .load(onAssetsLoaded)
            ...
            function onAssetsLoaded(loader, res) {
                ...
                newSkin.addSkin(hero.spineData.findSkin('body/body_' + i))
                newSkin.addSkin(hero.spineData.findSkin('weapon/weapon_' + i))
                newSkin.addSkin(hero.spineData.findSkin('hand/hand_' + i))
                if (i > 5) i = 5
                newSkin.addSkin(hero.spineData.findSkin('head/head_' + i))
                if (i > 2) i = 2
                newSkin.addSkin(hero.spineData.findSkin('common/common_' + i))
                hero.skeleton.setSkin(newSkin)
                // const newSkin1 = new currentSkinName('newSkin1')
                // newSkin1.addSkin(hero.spineData.findSkin('body/body_1'))
                // newSkin1.addSkin(hero.spineData.findSkin('weapon/weapon_1'))
                // newSkin1.addSkin(hero.spineData.findSkin('hand/hand_1'))
                // newSkin1.addSkin(hero.spineData.findSkin('head/head_1'))
                // newSkin1.addSkin(hero.spineData.findSkin('common/common_1'))
                // hero.skeleton.setSkin(newSkin1)
                // const newSkin2 = new currentSkinName('goblin')
                // newSkin2.addSkin(hero.spineData.findSkin('goblin'))
                // hero.skeleton.setSkin(newSkin2)
